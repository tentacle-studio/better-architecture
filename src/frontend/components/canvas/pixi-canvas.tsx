...
import { useWorkflowStore, generateEdgeId, type WorkflowNode, type WorkflowEdge } from "@/lib/workflow-store"
...
export function PixiCanvas() {
  ...
  const { zoom: canvasZoom, panOffset } = useCanvasStore()
  ...
  const draw = useCallback(() => {
    ...
    if (pendingPortRef.current) {
      ...
      // Glowing endpoint
      ctx.beginPath()
      ctx.arc(pp.currentX, pp.currentY, 5 / zoom, 0, Math.PI * 2)
    ...
    currentNodes.forEach((node) => {
      ...
      ctx.shadowBlur = isSelected ? 20 / zoom : 10 / zoom
      ...
      node.inputs.forEach((port, idx) => {
        ...
        ctx.font = `${9.5 / zoom}px -apple-system, sans-serif`
        ctx.textAlign = "left"
        ctx.textBaseline = "middle"
  ...
  const onPointerMove = useCallback((e: React.PointerEvent<HTMLCanvasElement>) => {
    const { nodes, zoom: z, panX: px, panY: py } = useWorkflowStore.getState()
    const world = screenToWorld(e.clientX, e.clientY)
